...
static enum mad_flow input(struct mad_stream *stream, player_t *player) {
    ...
    while (1) {
        ...
        if (readable_bytes == 0) {
            ...
            ESP_LOGD(TAG, "Buffer underflow, need %d bytes.", sizeof(read_buf) - rem);
            buf_underrun_cnt++;
...
void mp3_decoder_task(void *pvParameters)
{
    ...
    if (stream==NULL) { ESP_LOGE(TAG, "malloc(stream)"); return; }
    if (synth==NULL) { ESP_LOGE(TAG, "malloc(synth)"); return; }
    if (frame==NULL) { ESP_LOGE(TAG, "malloc(frame)"); return; }
