...
void dac_sample_signed(int16_t left, int16_t right) {
	uint32_t sample;
	left += 0x8000;
	right += 0x8000;
	sample = ((left << 16) & 0xffff0000) | right;
	i2s_push_sample(I2S_NUM_0, &sample, portMAX_DELAY);
}

void dac_sample_unsigned(uint16_t left, uint16_t right) {
	uint32_t sample;
	sample = ((left << 16) & 0xffff0000) | right;
	i2s_push_sample(I2S_NUM_0, &sample, portMAX_DELAY);
}
